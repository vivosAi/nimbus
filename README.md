...
make bundle    # universal (arm64 + x86_64) signed Nimbus.app in build/
...
This project therefore signs with a stable self-signed identity, `Nimbus Dev`,
which makes the grant survive rebuilds. It was created once with:

```sh
openssl req -x509 -newkey rsa:2048 -keyout key.pem -out cert.pem -days 3650 -nodes \
  -config build/cert/ext.cnf
openssl pkcs12 -export -inkey key.pem -in cert.pem -out nimbus.p12 \
  -name "Nimbus Dev" -passout pass:nimbus
security import nimbus.p12 -k ~/Library/Keychains/login.keychain-db \
...
then re-tick Nimbus under System Settings → Privacy & Security → Accessibility.
...
Nimbus asks for Accessibility permission only to read *which* window has
...
is a preferences plist in `~/Library/Preferences/com.vivasonico.nimbus.plist`.

## Deviations from the spec

- **Two SPM targets instead of one.** `NimbusKit` holds the pure logic
  (coordinate conversion, palettes, the flare curve, preferences) so it can be
  unit-tested; `Nimbus` is the AppKit/Metal shell. The spec's file layout is
  otherwise preserved.
