...
**Nimbus leverages attention psychology.** The colors rotate to prevent
habituation. Otherwise the ring would fade from your awareness.
...
The ring hides itself while you drag or resize a window, since you already know
which window you are dragging. It returns when the window comes to rest.
...
focus, only stacking order, and it is increasingly gated behind Screen
Recording, which is a far more invasive permission.
...
feel like buying me a coffee, that is very kind. Never expected though, and the
app will never ask.
